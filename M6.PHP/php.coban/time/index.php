...
<pre>
    kiểu time timestamp
    kiểu này thì k đọc đc chính xác là ngày tháng năm nào
    nhưng có thể tính toán như cộng trừ đc
    hàm time() lấy ra thời gian hiện tại nhưng ở dạng timestamp
</pre>
<?php echo "<br> time()" . time(); ?>
<pre>
    Vấn đề 1 : chuyển định dạng timestamp về kiểu
    định dạng time đọc đc
...
?>
<pre>
    Vấn đề 2: chuyển đổi định dạng time date() về timestamp
    giải quyết: strtotime()
    hàm này sẽ chuyển đổi 1 số chuỗi thành thời gian timestamp
    nhưng hàm này chỉ hỗ trợ 1 kiểu định dạng thời gian như :
    2019-01-31 : Y-m-d
    31-01-2019 : d-m-Y
    Nếu không phải kiểu định dạng nó hỗ trợ nó sẽ trả về false
    Quan sát kết quả của hàm var_dump() :
...
<pre>
    ...
    nếu k có tham số đc truyền vào nó sẽ lấy timestamp hiện tại
</pre>
